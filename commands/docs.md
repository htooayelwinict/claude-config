...
## Tech Stack
- **Backend (PHP):** Laravel 12, PHP 8.3+
...
| `src/api/routes.py` | FastAPI routers |
| ... | ... |
...
| Type | Convention | Example |
|------|------------|---------|
| Routers | snake_case files, include router | `users.py`, `router = APIRouter()` |
| Schemas | PascalCase Pydantic models | `UserCreate`, `UserRead` |
| Services | snake_case modules, PascalCase classes | `user_service.py`, `UserService` |
| Tests | snake_case files | `test_users.py` |
...
| `uv run <cmd>` | Run Python tool with uv (if available) |
```
