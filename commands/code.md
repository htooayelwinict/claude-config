...
#### Backend First (Python/FastAPI)
1. **Schemas** — Pydantic models for IO
   - Add to `src/schemas/`

2. **Models** — ORM models (SQLAlchemy/SQLModel)
   - Add to `src/models/`
...
4. **Routers** — API endpoints
   - Add to `src/api/`, include in `src/main.py`

5. **Migrations** — Database migrations (e.g., Alembic)
   - Configure revisions as needed
...
11. **Python Tests** — API/service tests
    ```bash
    python -m pytest
    ```
...
python -m pytest             # Python tests OK
ruff check .                 # Python lint OK
mypy .                       # Python typing OK
```
